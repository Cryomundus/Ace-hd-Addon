...
bool hscan = true;
bool vscan = false;
int scanlinethickness = 2;
double sparsitymult = 1.5;
double linesbrightness = 0.65;
...
vec3 posfilter = normalize(vec3(0, 1.0, 0)); // primary NVG color (positive exposure)
...
void main(){
	...
	int temppixel;
	temppixel = int(TexCoord.y * (textureSize(InputTexture,0).y / scanlinethickness)) % int(resfactor * sparsitymult);
	if(hscan && (resfactor > 1) && (temppixel == 0))
	{
		FragColor = vec4(color * linesbrightness, 1.0);
		return;
	}
	
	temppixel = int(TexCoord.x * (textureSize(InputTexture,0).x / scanlinethickness)) % int(resfactor * sparsitymult);
	if(vscan && (resfactor > 1) && (temppixel == 0))
	{
		FragColor = vec4(color * linesbrightness, 1.0);
		return;
	}
